...
| `.claude/settings.json` ＋ `.claude/hooks/push_gate.py` | 第 4 章 | matcher 是 `Bash`；`git push`（含 `--tags`、`cd x && git push`）→ exit 2 且 stderr 含「已擋下」；`git status`、`echo push`、壞 JSON → exit 0 |
...
2. `claude --worktree` 從 `origin/main` 長，本機沒 push 的工具鏈不會在裡面，所以先自己從本機 `main` 開好兩個 worktree（同名 `--worktree` 會直接進既有的）：
   ...
   終端機 A：`claude --worktree feat`，整段貼：
...
- **worktree 裡沒有 `.claude/` 或 `SPEC.md`**：步驟 1 沒 commit，或跳過步驟 2 的 `git worktree add` 直接 `claude --worktree`（它從 `origin/main` 長）。回主 checkout 確認 commit 了，再到 worktree 裡 `git merge main`。
...
- **驗收說 merge 節點不夠**：收斂用了 fast-forward。重置到工具鏈那個 commit（`git reset --hard <hash>`）再用 `--no-ff` merge 一次；分支還在就不用重做 worktree。
